...
void save_guilds() {
	for (Guild to_save : guilds) {
		if (to_save.guild_settings & Gtps3::SETTINGS_0) {
			...
			save_["unlock_mascot"] = to_save.unlocked_mascot;
...
int get_rank(ENetPeer* peer) {
	uint32_t guild_id = pInfo(peer)->guild_id;
	vector<Guild>::iterator x = find_if(guilds.begin(), guilds.end(), [guild_id](const Guild& a) { return a.guild_id == guild_id; });
	if (x != guilds.end()) {
		Guild* guild_information = &guilds[x - guilds.begin()];
		for (GuildMember member_search : guild_information->guild_members) {
			if (member_search.member_name == pInfo(peer)->tankIDName) {
				return member_search.role_id;
				break;
			}
		}
	}
...
int get_mascot(ENetPeer* peer) {
	uint32_t guild_id = pInfo(peer)->guild_id;
	vector<Guild>::iterator x = find_if(guilds.begin(), guilds.end(), [guild_id](const Guild& a) { return a.guild_id == guild_id; });
	if (x != guilds.end()) {
		Guild* guild_information = &guilds[x - guilds.begin()];
		return (65536 * guild_information->guild_mascot[1]) + guild_information->guild_mascot[0];
	}
	return 0;
}
...
namespace configures {
	std::string colorsName3(ENetPeer* peer) {
		return pInfo(peer)->name_color;
	}
}
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			for (GuildMember search_member : guild_information->guild_members) {
				if (to_lower(search_member.member_name) == to_lower(pInfo(currentPeer)->tankIDName)) {
					string display_color = configures::colorsName3(currentPeer);
